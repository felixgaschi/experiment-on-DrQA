...
class EmbeddingPart(nn.Module):
    ...
    def __init__(self, args, normalize=True):
        ...
        # RNN document encoder
        if self.args.use_cnn and args.use_transformer:
        ...
        if args.concat_rnn_layers:
            self.doc_hidden_size *= args.doc_layers
    ...
    def forward(self, x1, x1_f, x1_mask, x2, x2_mask):
        ...
        # Encode question with RNN + merge hiddens
        question_hiddens = self.question_rnn(x2_emb, x2_mask)
        return doc_hiddens, question_hiddens
    

class Discriminator(nn.Module):

    def __init__(self, args, doc_hidden_size, question_hidden_size, normalize=True):
        super(Discriminator, self).__init__()

        self.args = args
        self.normalizer = normalize

        self.linear1 = nn.Linear(question_hidden_size, doc_hidden_size)
        self.linear2 = nn.Linear(question_hidden_size, doc_hidden_size)
    ...
    def forward(self, x, y):
        ...
        logits = torch.cat([xWy_1, xWy_2], -1)

        if self.normalize:
            if self.training:
                # In training we output log-softmax for NLL
                alpha = F.log_softmax(logits, dim=-1)
            else:
