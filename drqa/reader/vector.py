...
def vectorize(ex, model, single_answer=False):
    ...
    if args.use_charemb:
        document_char = torch.LongTensor(
            [
                [
                    args.characters.index(c) 
                    if c in args.characters 
                    else len(args.characters) 
                    for c in token[:args.char_len]
                ] + 
                [
                    0 for _ in range(max(0, args.char_len - len(token)))
                ] 
            for token in ex["document"]]
        )
        question_char = torch.LongTensor(
            [
                [
                    args.characters.index(c) 
                    if c in args.characters 
    ...
    return document, features, question, document_char, question_char, start, end, ex['id']
...
def batchify(batch):
    ...
    document_char = [ex[3] for ex in batch]
    question_char = [ex[4] for ex in batch]
    ...
    if question_char is not None and question_char[0] is not None:
        ...
        for i, q in enumerate(question_char):
            x2_char[i, :q.size(0), :].copy_(q)
    else:
        x2_char = None

    # Maybe return without targets
    if len(batch[0]) == NUM_INPUTS + NUM_EXTRA:
        return x1, x1_f, x1_mask, x2, x2_mask, x1_char, x2_char, ids
    ...
    return x1, x1_f, x1_mask, x2, x2_mask, x1_char, x2_char, y_s, y_e, ids
